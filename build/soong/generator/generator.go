...
func init() {
	android.RegisterModuleType("clover_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = cloverExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = cloverExpandVariables(ctx, dep_file)
	...
	cmd := cloverExpandVariables(ctx, String(g.properties.Cmd))
